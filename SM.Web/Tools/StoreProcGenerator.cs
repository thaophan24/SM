...
namespace SM.Web.Tools
{
    ...
    {
        public static DataServiceClient ServiceProxy
        {
            get
            {
        ...
        public static string CreateDefault(object model)
        {
            ...
            var properties = CacheManager.Instance.GetCachedProperties(model.GetType());
            // just process not identity column
            var notIdentityProps = properties.Where(x => x.GetCustomAttribute<IdentityAttribute>() == null);
            var notIdentityColumns = columnTypes.Where(x => notIdentityProps.Any(y => y.Name == x.Name));
        ...
        private static string GenerateWhereCondition(IEnumerable<PropertyInfo> props)
        {
            ...
            return res.Trim().Trim("and".ToArray());
        }
    }
}